...
def train_model(model, train_loader, test_loader, criterion, optimizer, scheduler, num_epochs=100, patience=15):
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), "efficientnetb3_weights_acc.pth")
            torch.save(model, "efficientnetb3_model_acc.pth")
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            torch.save(model.state_dict(), "efficientnetb3_weights_loss.pth")
            torch.save(model, "efficientnetb3_model_loss.pth")
    ...
    plt.legend()
    plt.savefig("efficientnetb3_train_test_looss")
    plt.show()
...
def visualize_tsne(model, loader, classes):
    ...
    plt.savefig("efficientnetb3_tsne_after_512.png")
    plt.show()
    ...
    plt.savefig("efficientnetb3_tsne_after_256.png")
    plt.show()
...
best_model = torch.load("efficientnetb3_model_acc.pth", weights_only = False).to(dev)
